...
contract RedemptionVault is ManageableVault, IRedemptionVault {
    ...
    uint256 public constant COUPON_UNITS_PER_USDC_BASE_UNIT = 1e12;
    ...
    /**
     * @notice coupon supply approved during initialization
     */
    uint256 public initialCouponSupply;
    ...
    function __RedemptionVault_init(
        ...
        require(
            IERC20Metadata(_mTokenInitParams.mToken).decimals() == 18,
            "RV: coupon decimals != 18"
        );
        require(
            _instantInitParams.instantDailyLimit %
                COUPON_UNITS_PER_USDC_BASE_UNIT ==
                0,
            "RV: supply not USDC-aligned"
        );
        require(
            IERC20(_mTokenInitParams.mToken).totalSupply() ==
                _instantInitParams.instantDailyLimit,
        ...
        require(
            _fiatRedemptionInitParams.fiatAdditionalFee == 0 &&
                _fiatRedemptionInitParams.fiatFlatFee == 0,
            "RV: fiat fee != 0"
        );
        ...
        initialCouponSupply = _instantInitParams.instantDailyLimit;
    }
    ...
    function redeemInstant(
        ...
        require(paused(), "RV: permanent pause disabled");
        require(greenlistEnabled, "RV: greenlist disabled");
        require(
            instantDailyLimit == initialCouponSupply,
            "RV: daily limit changed"
        );
        require(
            IERC20(address(mToken)).totalSupply() <= initialCouponSupply,
            "RV: supply exceeds initial"
        );
        require(
            minAmount == COUPON_UNITS_PER_USDC_BASE_UNIT,
            "RV: minimum changed"
        );
        require(instantFee == 0, "RV: instant fee changed");
        require(
            IERC20Metadata(address(mToken)).decimals() == 18,
            "RV: coupon decimals != 18"
        );
        require(
            IERC20Metadata(tokenOut).decimals() == 6,
            "RV: token decimals != 6"
        );
        ...
        require(
            amountMTokenIn % COUPON_UNITS_PER_USDC_BASE_UNIT == 0,
            "RV: amount not USDC-aligned"
        );
        require(
            minReceiveAmount == amountMTokenIn,
            "RV: min receive must be exact"
        );
        ...
        )
    {
        ...
        ICouponToken(address(mToken)).burnFrom(
            user,
